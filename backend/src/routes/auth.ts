...
export async function authRoutes(app: FastifyInstance) {
  app.post('/login', { config: { rateLimit: { max: 10, timeWindow: '1 minute' } } }, async (req, reply) => {
    ...
    if (!user) {
      await writeAudit(app.prisma, { entity: 'auth', action: 'LOGIN_FAILED', details: { email: body.email, reason: 'unknown email' }, ip: req.ip });
      return reply.code(401).send({ error: 'Invalid credentials' });
    }
    if (!user.active) {
      await writeAudit(app.prisma, { userId: user.id, entity: 'auth', action: 'LOGIN_BLOCKED', details: { email: body.email, reason: 'deactivated' }, ip: req.ip });
      return reply.code(403).send({ error: 'Account is deactivated.' });
    }
    const ok = await bcrypt.compare(body.password, user.passwordHash);
    if (!ok) {
      await writeAudit(app.prisma, { userId: user.id, entity: 'auth', action: 'LOGIN_FAILED', details: { email: body.email, reason: 'bad password' }, ip: req.ip });
      return reply.code(401).send({ error: 'Invalid credentials' });
    }
    ...
    return {
      token,
      user: { id: user.id, email: user.email, name: user.name, role: user.role, branchId: user.branchId, branchName: user.branch?.name ?? null },
    };
  });
  ...
  app.get('/me', { preHandler: authGuard }, async (req) => {
    const user = await app.prisma.user.findUnique({
      ...
      select: { id: true, email: true, name: true, role: true, branchId: true, branch: { select: { name: true } } },
    });
  ...
  app.get('/users', { preHandler: [authGuard, requireRole('ADMIN')] }, async () =>
    app.prisma.user.findMany({
      select: { id: true, email: true, name: true, role: true, active: true, branchId: true, branch: { select: { name: true } } },
      orderBy: { name: 'asc' },
    }),
  );
  ...
  app.post('/users', { preHandler: [authGuard, requireRole('ADMIN')] }, async (req, reply) => {
    ...
    const created = await app.prisma.user.create({
      data: { email: body.email, name: body.name, role: body.role, branchId: body.branchId ?? null, passwordHash },
      select: { id: true, email: true, name: true, role: true, active: true, branchId: true },
    });
